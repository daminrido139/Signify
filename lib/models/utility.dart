class Utility {
  static const List youtubeurl = [
    "vnH2BmcSRMA",
    "qcdivQfA41Y",
    "VtbYvVDItvg",
    "qtrBGmioR2Q",
    "drs0_jcKr5w",
    "XPRtZQSKL-4",
    "x58C6-ZtW_8",
  ];
  static const List verticallst = [
    ["assets/img/learnimg3.webp", "NUMBERS", "(0-1000)"],
    ["assets/img/learnimage2.webp", "ALPHABETS", "(A-Z)"],
    ["assets/img/learnimag1.webp", "WORDS", "(eg.Tiger)"],
    ["assets/img/color.jpeg", "Colors", "(eg.blue)"],
    ["assets/img/relation.jpeg", "Relations", "(eg.Father)"],
    ["assets/img/weaks.jpeg", "Days of the Weak", "(eg.friday)"],
    ["assets/img/month.jpeg", "Months", "(eg.Jan)"],
  ];
  static const List horizontallst = [
    "assets/img/class1.jpeg",
    "assets/img/class2.jpeg",
    "assets/img/class3.jpeg",
    "assets/img/class4.jpeg",
    "assets/img/class5.jpeg",
    "assets/img/class6.jpeg",
    "assets/img/class7.jpeg",
    "assets/img/class8.jpeg",
  ];
}
