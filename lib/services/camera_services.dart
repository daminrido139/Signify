...
class CameraServices {
  static Uint8List convertYUV420toImageColor(CameraImage image) {
  ...
  static Future<String> predictGesture(List<CameraImage> images) async {
    print(images.length.toString());
    String res = '';
    final Uri url = Uri.parse('http://192.168.51.41:5000/upload');
    ...
    for (int i = 0; i < images.length; i++) {
      var multipartFile = http.MultipartFile.fromBytes(
        ...
        convertYUV420toImageColor(images[i]),
    ...
    if (response.statusCode == 200) {
      res = jsonDecode(await response.stream.bytesToString())['res'].toString();
    } else {
